...
// base directory with a different leaf glob. The cached arm performs one OS
// read per shared scan round; the uncached arm performs one per input.
func BenchmarkGetFilesSharedDir(b *testing.B) {
	for _, cached := range []bool{false, true} {
		for _, inputs := range []int{10, 100, 400} {
			b.Run(fmt.Sprintf("cached=%t/inputs=%d", cached, inputs), func(b *testing.B) {
				base := b.TempDir()
				...
				for i := range inputs {
					...
					var err error
					scanners[i], err = newFileScannerWithCache(logp.NewNopLogger(), []string{glob}, cfg, CompressionNone, dc)
					require.NoError(b, err)
				}
